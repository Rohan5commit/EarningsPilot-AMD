...
If vLLM remains blocked by container entrypoint issues, use `./scripts/amd/serve-transformers-openai-rocm.sh`. It loads the base model plus optional PEFT adapter with Transformers/FastAPI and exposes the same OpenAI-compatible `/v1/models` and `/v1/chat/completions` endpoints expected by `npm run benchmark:amd`. The fallback creates/uses `.venv` with system site packages by default to avoid `externally-managed-environment` pip failures while preserving access to ROCm Torch, is intentionally capped for demo reliability (`MAX_MODEL_LEN=1024`, `MAX_NEW_TOKENS_DEFAULT=48` by default), uses `device_map=auto` on GPU hosts, merges the LoRA adapter before serving when possible, and exposes `/health`; wait for `curl http://<gpu-host>:8000/health` to return `ok` before launching the benchmark. If real generation still approaches the benchmark timeout, lower `BENCHMARK_MAX_TOKENS` (for example `BENCHMARK_MAX_TOKENS=48`), raise `BENCHMARK_TIMEOUT_MS`, or leave the default `FALLBACK_RESPONSE_MODE=auto` so JSON-object benchmark requests use the fast grounded template path; set `FALLBACK_RESPONSE_MODE=generate` to force model generation.
